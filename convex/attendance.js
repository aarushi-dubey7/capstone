...
function isLateForTime(startTime) {
...
export const markPresent = mutation({
    ...
    handler: async (ctx, { studentId, locationUuid, locationName }) => {
        ...
        const schedule = await ctx.db
            ...
            .filter((q) => q.and(q.eq(q.field("dayOfWeek"), dayOfWeek), q.eq(q.field("room"), locationName.replace("Room ", "").trim())))
            .first();
...
export const getStudentLogs = query({
    ...
    handler: async (ctx, { studentId, date }) => {
        return ctx.db
            .query("logs")
            .withIndex("by_student_date", (q) => q.eq("studentId", studentId).eq("date", date ?? todayString()))
            .order("asc")
            .collect();
    },
});
