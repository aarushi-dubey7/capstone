...
export default function TeacherDashboard() {
  ...
  const allRoomCodes    = useMemo(() => [...new Set(allSchedules.map((s) => s.room))].sort(), [allSchedules]);
  const locationByRoom  = useMemo(() => new Map(allLocations.map((l) => [l.roomNumber, l])), [allLocations]);
  const subjectsByRoom  = useMemo(() => {
    const m = new Map<string, Set<string>>();
    for (const s of allSchedules) {
      if (!m.has(s.room)) m.set(s.room, new Set());
      m.get(s.room)!.add(s.subject);
    }
    return m;
  }, [allSchedules]);
        ...
        {tab === "rooms" && (
          <div className="space-y-4">
            <p className="text-sm text-slate-500">
              All rooms found in student schedules. Assign a BLE beacon to each so check-in works automatically.
            </p>
            {allRoomCodes.length === 0 ? (
              <div className="card text-center py-10 text-slate-400">No student schedules on file yet</div>
            ) : (
                    ...
                    <div key={code} className="card space-y-3">
                      ...
                      {isEditing && (
                        <div className="space-y-2">
                          ...
                          <input value={roomDeviceName} onChange={(e) => setRoomDeviceName(e.target.value)} placeholder="BLE device name (e.g. Attendance-Anchor-101)"
                            className="w-full border border-slate-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
